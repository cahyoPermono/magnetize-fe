          ...
          <TabPanel header="Register">
            <Form v-slot="{ meta }" @submit="save" :validation-schema="schema_2">
              <div class="px-5">
                ...
                <div class="grid">
                  <div class="col-12">
                    <label for="name"> <small>Nama</small><span style="color: red">*</span> </label>
...
const save = async (values, { resetForm }) => {
  try {
    const data = await axios.post(config.API_BASE_URL + "applicant_auth/register", values);
...
const onSubmit = async (values) => {
  try {
    const data = await axios.post(config.API_BASE_URL + "applicant_auth/login", values);
