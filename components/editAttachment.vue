<template>
  <Button icon="pi pi-pi-pencil" @click="openPosition('top')" class="p-button-warning" style="float: right" />

  <Dialog header="Edit Note" v-model:visible="displayPosition" :breakpoints="{ '960px': '75vw', '640px': '90vw' }"
    :style="{ width: '50vw' }" :position="position" :modal="true">
    <Form class="mb-3" @submit="save">
      <div class="card p-3">
        <div class="">
          <label for="edit_note"><small>Note</small><span style="color: red">*</span></label>
          <Field class="form-control" name="note" :rules="isRequired" v-model="Notes.note" />
